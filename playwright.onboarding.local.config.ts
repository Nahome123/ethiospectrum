...
function getLocalSupabaseEnvironment() {
  ...
  try {
    ...
    throw new Error(
      "Local Supabase is unavailable. Start the local stack before running onboarding E2E tests.",
    );
  }
  ...
  if (!url || !publishableKey || !secretKey || !localSupabaseUrl.test(url)) {
    throw new Error("Refusing to run onboarding E2E tests without a local Supabase API target.");
  }

  return {
    E2E_LOCAL_SUPABASE: "1",
    E2E_ONBOARDING_LOCAL_CONFIG: "1",
...
export default defineConfig({
  testDir: "./tests/e2e",
  testMatch: "onboarding.local.spec.ts",
  fullyParallel: false,
  workers: 1,
  timeout: 120_000,
