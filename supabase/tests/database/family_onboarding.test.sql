...
select has_function('public', 'complete_household_onboarding', array['text', 'text', 'text', 'text', 'text', 'text'], 'complete_household_onboarding function exists');

set local role anon;
select throws_ok($$select public.complete_household_onboarding('Anonymous household', '2026-07-15', 'Caregiver', null, 'en', 'UTC')$$, '42501', null, 'unauthenticated onboarding is rejected');

reset role;
set local role authenticated;
set local request.jwt.claim.sub = '20000000-0000-0000-0000-000000000001';
select isnt(public.complete_household_onboarding('  Onboarding household  ', '2026-07-15', 'Caregiver', null, 'en', 'UTC')::text, null, 'onboarding returns the created household ID');
...
select is(public.complete_household_onboarding('Second household', '2026-07-15', 'Caregiver', null, 'en', 'UTC'), (select id from public.households where primary_owner_id = '20000000-0000-0000-0000-000000000001'), 'repeat onboarding returns the existing household');
...
select isnt(public.complete_household_onboarding('Second household', '2026-08-01', 'Caregiver', null, 'en', 'UTC')::text, null, 'a newer policy version can be accepted after onboarding');
...
select throws_ok($$select public.complete_household_onboarding('', '2026-07-15', 'Caregiver', null, 'en', 'UTC')$$, '22023', null, 'an empty household name is rejected');
select throws_ok($$select public.complete_household_onboarding('   ', '2026-07-15', 'Caregiver', null, 'en', 'UTC')$$, '22023', null, 'a whitespace-only household name is rejected');
select throws_ok($$select public.complete_household_onboarding('Onboarding household', '  ', 'Caregiver', null, 'en', 'UTC')$$, '22023', null, 'a blank consent policy version is rejected');
