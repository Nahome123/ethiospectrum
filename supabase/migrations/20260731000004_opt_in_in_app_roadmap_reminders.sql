...
create or replace function public.update_roadmap_item_and_reschedule_reminders(
  target_item_id uuid, expected_updated_at timestamptz, input_title text, input_description text,
  input_category text, input_priority text, input_status text, input_due_date date,
  input_dependent_id uuid, input_assigned_to uuid, input_reminder_schedules jsonb
) returns table(id uuid, updated_at timestamptz)
language plpgsql security definer set search_path = '' as $$
