...
`ETH-016 Add OCR fallback`: evaluate scanned-PDF support only as a separate provider, privacy, quality, retention, authorization, and accessibility review. It must not be used to relabel a `needs_ocr` document as processed, and it must not add document chat, general AI, public sharing, or unrestricted text search.
