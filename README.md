...
The public education-support experience adds the localized `/[locale]/resources/education` learning guide and a landing-page path to it. It provides general IEP, evaluation, classroom-support, meeting-preparation, and autism-support learning material with an educational-only disclaimer. Landing-page personalisation derives only a safe visitor/onboarding/ready state on the server; it does not render names, household details, dependent data, school details, health information, or analytics payloads. The assistant route is linked only as an existing signed-in product entry point, while membership tools remain an informational preview because subscription billing is not implemented.
...
`ETH-019 Implement multilingual document chat`: it may consume ETH-018 review status, scores, and citation-quality warnings, but it must remain a separate, language-appropriate, cited document-chat scope.
