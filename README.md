...
ETH-027 adds request-level appointments on top of the ETH-026 grant. Every appointment belongs to one support request, inherits that request's household, and is tied to the request's currently assigned specialist. It never creates household-wide or specialist-global visibility. The dormant `appointments` table is reused, but its foundation policy, which read `can_access_household()` and so honoured dormant household-wide specialist rows, is dropped and replaced with request-scoped policies plus forced RLS.

The assigned specialist proposes one time: a local date and time, a confirmed IANA timezone, a 30/45/60-minute duration, a video or phone modality, and for video a required HTTPS meeting link. PostgreSQL converts the local time to a UTC instant and refuses to guess: an invalid zone, a nonexistent spring-forward time, or an ambiguous fall-back time is rejected rather than silently resolved. Proposals must start at least 24 hours out and within 90 days, must not overlap another live appointment for the same specialist or household (half-open intervals, so back-to-back is allowed), and only one proposed or scheduled appointment may exist per request.

The household consents explicitly through an unchecked checkbox bound to the exact appointment version, and the server records the consenting user, timestamp, and controlled consent-copy version. Owners, household administrators, and members may accept, decline, cancel, or ask for another time; viewers are read-only; and the household never picks the time. Rescheduling is deliberately not an in-place edit: the live appointment is cancelled with reason `reschedule_requested`, preserved, and the specialist proposes a replacement linked through `supersedes_appointment_id`. Once scheduled, the UTC instant is fixed and a later daylight-saving transition never moves it; the appointment is displayed in its confirmed timezone, with the viewer's profile timezone shown alongside when it differs.

Specialists may cancel and, after the start time, mark an appointment completed; they can never consent for the household or accept their own proposal. Platform administrators observe status, timing, consent state, and immutable history but cannot propose, accept, decline, reschedule, cancel, or complete. Revoking the assignment or closing or cancelling the request cancels any live appointment in the same trusted transaction, with reasons `assignment_revoked`, `request_closed`, or `request_cancelled`. The meeting link is released only to the household and assigned specialist, only once scheduled, and never appears in audit metadata, administrator queues, or logs. ETH-027 adds no notifications, recurring appointments, availability calendars, slot workers, external calendar integration, in-person modality, or environment variables. Native Amharic and Spanish review remains required before release.

## ETH-026 specialist assignment

ETH-026 adds the deliberate specialist grant ETH-025 left out. Assignment is **request-level**: the authoritative record is the existing `support_threads.specialist_id` column, extended with `specialist_assigned_at`, `specialist_assigned_by`, a server-controlled `assignment_version` starting at zero, and `assignment_updated_at`. A request holds at most one active specialist; a specialist may hold many requests. `household_specialists` stays dormant — ETH-026 neither populates nor consults it, and `can_access_household()` is deliberately not the authorization boundary because it would grant household-wide access.

Only a platform administrator assigns or revokes, from `/[locale]/admin/support-requests/[requestId]`, choosing from the directory at `/[locale]/admin/specialists`. An eligible specialist holds the global `specialist` role, has a specialist profile, and is marked `available`. Assignment requires an open, unassigned request and a matching expected assignment version; revocation requires an active assignment and a matching version. Households cannot assign, revoke, select, or request a specialist — they see only the assigned specialist's safe display name and any specialist responses. Administrators never send messages, close, or cancel through ETH-026, and the global content-editor role grants nothing.

An actively assigned specialist works at `/[locale]/specialist/support-requests`, seeing only their own assigned open requests: subject, category, preferred language, dates, message count, caregiver-safe attribution, the message thread, and a response form. They cannot read any other request, household membership, dependents, documents, roadmap items, reminders, or assignment history, and cannot close, cancel, reassign, or edit anything. Specialist responses reuse the ETH-025 message log with a server-derived `author_kind` of `specialist` (existing messages backfilled as `caregiver`), count toward the same 50-message cap, and remain immutable.

Revocation is immediate: RLS re-derives access from live database state on every query, so an existing browser session or unexpired JWT loses read and write access the moment the assignment ends. Closing or cancelling a request atomically clears the assignment, increments the version, and records a `request_closed` or `request_cancelled` revocation; administrator revocation records `administrator_revoked`. All of it lands in the append-only, administrator-only `support_request_assignment_events` table, which stores no message body or household-private content and survives revocation. A revoked specialist may be assigned again as a new assignment; there is no reactivation state, no expiry, and no workload cap. ETH-026 adds no notifications and no ETH-027 appointment scheduling. Native Amharic and Spanish review remains required before release.
...
With ETH-025 and ETH-026 implemented locally, ETH-027 appointment scheduling is the next recommended issue: timezone-aware availability and booking consent, building on the request-level specialist relationship ETH-026 established. ETH-026 deliberately ships without any appointment, availability, scheduling, or notification behavior.

For a local ETH-026 check, use synthetic accounts only. As a platform administrator, review the specialist directory, assign an available specialist to an open request, and confirm the assignment, version, and history appear. Confirm household owners, household administrators, members, viewers, specialists, and content editors are all denied the assignment controls. As the assigned specialist, confirm the workload lists only that request, that a response is delivered and labelled as a specialist response for the household, and that dependents, documents, roadmap items, reminders, and other households remain unreachable. Revoke the assignment and confirm list and direct-route access disappear immediately without a re-login, then close another assigned request and confirm the automatic revocation and its audit reason. Verify a stale assignment version shows the refresh conflict, that an active `household_specialists` row alone grants nothing, and that all three locales render. Run `pnpm test:e2e:specialists:local` only against local Supabase.
