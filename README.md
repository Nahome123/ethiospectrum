...
For a local ETH-019 check, use only a synthetic completed document with chunks. Start an English, Amharic, and Spanish conversation as a non-viewer; complete queued responses through a mocked provider or the service-only synthetic completion function; refresh and reopen each conversation; verify a viewer is read-only; and confirm removed/unrelated users and archived documents are denied. Verify ordinary answers show only Source/Page labels, not excerpts or page links; test insufficient-evidence and out-of-document fallback results, a failed response retry, one pending response per conversation, keyboard composition, narrow layout, and 200% zoom. Do not use a live provider or personal document.
...
`ETH-020 Add document-page citations` remains next: it may use ETH-019's stored validated coordinates, but it alone owns citation excerpts, links, previews, page navigation, and inaccessible-source handling.
