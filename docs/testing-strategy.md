...
ETH-010 unit coverage validates trimmed multilingual names, optional-name normalization, supported locales, IANA timezones, consent, untrusted browser ID/role omission, logged-out action denial, safe persistence failures, locale-aware redirects, shared household resolution, and Server Action export boundaries. Its pgTAP test uses only synthetic Auth users to verify the profile update, consent, one household, one active owner membership, repeat/direct-RPC idempotency, direct membership-write denial, transaction rollback, fixed function search path, RLS, and removal revocation. Run `pnpm test:e2e:onboarding:local` only with a local Supabase stack; it rejects non-local endpoints and uses isolated synthetic accounts for localized redirects, form accessibility, Amharic names, duplicate checks, dashboard display, and cross-household isolation.
