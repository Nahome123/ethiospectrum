...
ETH-012 adds protected document routes at `/[locale]/documents`, `/[locale]/documents/upload`, and `/[locale]/documents/[documentId]`, plus the authenticated download route at `/api/documents/[documentId]/download`. The documents list and dashboard load only the current active household's RLS-protected rows. The list defaults to active uploaded documents and separately labels pending, failed, and archived records when the caller has archive visibility; no page claims that a file has been processed. The upload page server-loads only active dependents from the verified household and never receives a household ID, actor ID, document ID, or object path from the browser.
